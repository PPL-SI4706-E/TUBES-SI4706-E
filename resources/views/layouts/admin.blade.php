...
<div class="flex h-screen overflow-hidden relative">
    ...
    <aside class="fixed inset-y-0 left-0 z-50 w-64 bg-gradient-to-b from-sky-800 via-sky-800 to-sky-900 text-white flex flex-col transition-transform duration-300 lg:static lg:translate-x-0"
        ...
        <nav class="flex-1 p-3 space-y-0.5 overflow-y-auto">
            ...
            @php
                $navItems = [
                    ...
                    ['route' => 'notifikasi.index',           'label' => 'Notifikasi',         'icon' => 'bell'],
                ];
            @endphp
            ...
            @foreach($navItems as $item)
                ...
                <a href="{{ route($item['route']) }}"
                   class="flex items-center justify-between px-3 py-2.5 rounded-lg transition-all
        ...
        <header class="bg-white border-b border-slate-200 h-14 flex items-center justify-between px-6 shrink-0">
            ...
            <div class="flex items-center gap-3">
                ...
                <div class="relative" x-cloak>
                    {{-- Bell Button --}}
                    <button id="admin-notif-bell"
                            @click="open = !open; if(open) fetchDropdown()"
                            class="relative p-2 rounded-lg text-slate-500 hover:text-sky-700 hover:bg-sky-50 transition-colors">
                        <i data-lucide="bell" class="w-5 h-5"></i>
                        <span x-show="unreadCount > 0"
                              x-text="unreadCount > 9 ? '9+' : unreadCount"
                              class="absolute -top-0.5 -right-0.5 min-w-[18px] h-[18px] bg-red-500 text-white text-[10px] font-bold rounded-full flex items-center justify-center px-1 shadow"
                              id="admin-notif-badge"></span>
                    </button>
                    ...
                    <div x-show="open"
                         ...
                         class="absolute right-0 mt-2 w-80 bg-white rounded-xl shadow-xl border border-slate-100 overflow-hidden z-50 origin-top-right"
                         id="admin-notif-dropdown">

                        {{-- Dropdown Header --}}
                        <div class="px-4 py-3 border-b border-slate-100 flex items-center justify-between bg-slate-50/60">
                            <a href="{{ route('notifikasi.index') }}" class="font-semibold text-slate-800 text-sm hover:text-sky-600 transition-colors">
                                Notifikasi
                            </a>
                            <div class="flex gap-3">
                                <button x-show="unreadCount > 0" @click="dropdownReadAll()"
                                        class="text-[11px] text-sky-600 hover:text-sky-700 font-medium flex items-center gap-1 transition-colors">
                                    <i data-lucide="check-check" class="w-3 h-3"></i> Tandai semua dibaca
                                </button>
                                <button x-show="items.length > 0" @click="dropdownClearAll()"
                                        class="text-[11px] text-red-500 hover:text-red-600 font-medium flex items-center gap-1 transition-colors">
                        ...
                        {{-- Items --}}
                        <div class="max-h-[350px] overflow-y-auto divide-y divide-slate-50" id="admin-notif-list">
                            {{-- Loading --}}
                            <div x-show="loadingDrop" class="p-6 flex items-center justify-center text-slate-400 text-sm gap-2">
                            ...
                            {{-- Empty --}}
                            <div x-show="!loadingDrop && items.length === 0"
                                 class="p-8 text-center text-slate-400 text-sm flex flex-col items-center">
                                <i data-lucide="bell-off" class="w-10 h-10 mb-2 text-slate-300"></i>
                                Belum ada notifikasi baru
                            </div>
                            {{-- Notification Items --}}
                            <template x-for="notif in items" :key="notif.id">
                                <div :class="!notif.read ? 'bg-sky-50/30' : ''"
                                     class="relative px-4 py-3 flex gap-3 group hover:bg-slate-50 transition-colors">
                                    {{-- Unread bar --}}
                                    <div x-show="!notif.read" class="absolute left-0 top-1/2 -translate-y-1/2 w-1 h-8 bg-sky-500 rounded-r-sm"></div>
                                    {{-- Avatar --}}
                                    <div :class="!notif.read ? 'bg-white shadow-sm border border-sky-100' : 'bg-slate-100'"
                                         class="w-9 h-9 rounded-full flex items-center justify-center shrink-0 mt-0.5">
                                        <i :data-lucide="notif.type === 'new_task' ? 'wrench' : notif.type === 'payment_needed' ? 'alert-circle' : 'bell'"
                                           :class="notif.type === 'new_task' ? 'text-sky-500' : notif.type === 'payment_needed' ? 'text-orange-500' : 'text-indigo-500'"
                                           class="w-4 h-4"></i>
                                    </div>
                                    {{-- Text --}}
                                    <div class="flex-1 min-w-0 cursor-pointer pr-6"
                                         @click="if(!notif.read) dropdownMarkRead(notif.id); if(notif.link) window.location.href = notif.link;">
                                        <p :class="!notif.read ? 'font-semibold text-slate-900' : 'font-medium text-slate-700'"
                                           class="text-sm leading-tight truncate" x-text="notif.title"></p>
                                        <p class="text-xs text-slate-500 mt-0.5 line-clamp-2 leading-relaxed" x-text="notif.message"></p>
                                        <span class="text-[10px] text-slate-400 block mt-1" x-text="relTime(notif.created_at)"></span>
                                    </div>
                                    {{-- Delete hover --}}
                                    <button @click.stop="dropdownDelete(notif.id)"
                                            class="absolute right-2 top-2 p-1 rounded text-slate-400 hover:text-red-500 hover:bg-red-50 opacity-0 group-hover:opacity-100 transition-all">
                        ...
                        <div class="p-3 border-t border-slate-100 bg-slate-50/60">
                            <a href="{{ route('notifikasi.index') }}"
                               class="block w-full text-center text-sm font-medium text-sky-600 hover:text-sky-700 py-1 transition-colors">
...
{{-- PBI-18: Shared notifBell() Alpine component (navbar bell dropdown) --}}
<script>
function notifBell() {
    return {
        ...
        async fetchUnread() {
            try {
                const r = await fetch('/api/notifications?filter=unread', { headers: this._h() });
        ...
        async fetchDropdown() {
            ...
            try {
                const r = await fetch('/api/notifications', { headers: this._h() });
                const j = await r.json();
                this.items = (j.notifications ?? []).slice(0, 5);
                this.unreadCount = j.unread_count ?? 0;
                this.$nextTick(() => { if (window.lucide) lucide.createIcons(); });
            } catch(e) {}
            finally { this.loadingDrop = false; }
        },
        async dropdownMarkRead(id) {
            try {
                const r = await fetch('/api/notifications/' + id + '/read', { method: 'PATCH', headers: this._h('PATCH') });
                const j = await r.json();
                const n = this.items.find(i => i.id === id);
                if (n) n.read = true;
                this.unreadCount = j.unread_count ?? Math.max(0, this.unreadCount - 1);
            } catch(e) {}
        },
        async dropdownReadAll() {
            try {
                await fetch('/api/notifications/read-all', { method: 'POST', headers: this._h('POST') });
                this.items.forEach(n => n.read = true);
                this.unreadCount = 0;
            } catch(e) {}
        },
        async dropdownDelete(id) {
            try {
                const r = await fetch('/api/notifications/' + id, { method: 'DELETE', headers: this._h('DELETE') });
                const j = await r.json();
                this.items = this.items.filter(i => i.id !== id);
                this.unreadCount = j.unread_count ?? this.unreadCount;
            } catch(e) {}
        },
        async dropdownClearAll() {
            if (!confirm('Hapus semua notifikasi?')) return;
            try {
                await fetch('/api/notifications/clear-all', { method: 'DELETE', headers: this._h('DELETE') });
                this.items = []; this.unreadCount = 0;
            } catch(e) {}
        },
    };
}
