            ...
            <div class="flex items-center gap-3">
                ...
                <div class="relative" x-cloak>
                    ...
                    <div x-show="open" @click.outside="open = false"
                        ...
                        <div class="max-h-[350px] overflow-y-auto divide-y divide-slate-50" id="warga-notif-list">
                            <div x-show="loadingDrop" class="p-6 flex items-center justify-center text-slate-400 text-sm gap-2">
                                <svg class="animate-spin w-4 h-4 text-sky-500" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24"><circle class="opacity-25" cx="12" cy="12" r="10" stroke="currentColor" stroke-width="4"/><path class="opacity-75" fill="currentColor" d="M4 12a8 8 0 018-8V0C5.373 0 0 5.373 0 12h4z"/></svg>
                                Memuat...
                            </div>
                            <div x-show="!loadingDrop && items.length === 0" class="p-8 text-center text-slate-400 text-sm flex flex-col items-center">
                            ...
                            <template x-for="notif in items" :key="notif.id">
                                <div :class="!notif.read ? 'bg-sky-50/30' : ''" class="relative px-4 py-3 flex gap-3 group hover:bg-slate-50 transition-colors">
                                    ...
                                    <div :class="!notif.read ? 'bg-white shadow-sm border border-sky-100' : 'bg-slate-100'" class="w-9 h-9 rounded-full flex items-center justify-center shrink-0 mt-0.5">
                                        <i :data-lucide="notif.type === 'new_task' ? 'wrench' : notif.type === 'payment_needed' ? 'alert-circle' : 'bell'" :class="notif.type === 'new_task' ? 'text-sky-500' : notif.type === 'payment_needed' ? 'text-orange-500' : 'text-indigo-500'" class="w-4 h-4"></i>
                                    </div>
