...
@section('content'){{-- TODO --}}@endsection
=======
@extends('layouts.admin')
...
<div>
    ...
    <div class="grid grid-cols-2 lg:grid-cols-3 gap-4 mb-6">
        @php
            $cards = [
                ...
                ['label' => 'Testimoni Menunggu Validasi', 'value' => $stats['testimoni_pending'], 'icon' => 'messages-square', 'color' => 'bg-teal-500', 'bgLight' => 'bg-teal-50', 'textColor' => 'text-teal-700'],
            ];
        @endphp
    ...
    <div class="bg-white rounded-xl p-6 border border-sky-100 shadow-sm mb-6">
        <div class="flex items-center justify-between gap-4 mb-4">
            <div>
                <h3 class="text-sky-800" style="font-size: 1rem; font-weight: 600;">Testimoni Terbaru</h3>
                <p class="text-slate-500 mt-1" style="font-size: 0.8rem;">Pantau masukan publik yang baru masuk tanpa perlu pindah menu dulu.</p>
            </div>
            <a href="{{ route('admin.testimoni.index') }}" class="text-sky-600 hover:text-sky-800 font-semibold" style="font-size: 0.85rem;">Buka Moderasi</a>
        </div>
        <div class="space-y-3">
            @forelse($recentTestimoni as $testimoni)
                @php
                    $statusStyles = [
                ...
                @endphp
                <div class="border border-sky-50 rounded-xl p-4">
                    <div class="flex flex-wrap items-center justify-between gap-3 mb-2">
                        <div>
                            <p class="text-sky-900" style="font-size: 0.9rem; font-weight: 600;">{{ $testimoni->nama }}</p>
                            <p class="text-slate-400" style="font-size: 0.75rem;">{{ $testimoni->created_at?->format('d M Y H:i') }}</p>
                        </div>
                        <span class="{{ $statusStyles[$testimoni->status_validasi] ?? 'bg-slate-100 text-slate-700' }} px-3 py-1 rounded-full inline-flex items-center justify-center" style="font-size: 0.72rem; font-weight: 700;">
                            {{ $statusLabels[$testimoni->status_validasi] ?? $testimoni->status_validasi }}
                        </span>
                    </div>
                    <p class="text-slate-600" style="font-size: 0.82rem; line-height: 1.6;">{{ $testimoni->pesan }}</p>
                </div>
            @empty
                <div class="rounded-xl border border-dashed border-sky-200 p-6 text-center text-slate-500" style="font-size: 0.82rem;">
                    Belum ada testimoni publik yang masuk.
                </div>
            @endforelse
        </div>
    </div>
