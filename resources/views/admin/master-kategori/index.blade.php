            ...
            <div class="bg-white rounded-xl p-5 border border-sky-100 shadow-sm hover:shadow-md transition-shadow">
                <div class="flex items-start justify-between">
                    ...
                    <div class="flex gap-1">
                        <button data-action="edit" data-id="{{ $k->id }}" @click="showForm = true; editing = {{ $k->id }}; form = { nama_kategori: '{{ addslashes($k->nama_kategori) }}', deskripsi: '{{ addslashes($k->deskripsi) }}', tarif: {{ $k->tarif }}, icon: '{{ $k->icon ?? '📋' }}' }"
                        ...
                        <form action="{{ route('admin.master-kategori.destroy', $k) }}" method="POST" onsubmit="return confirm('Yakin hapus kategori ini?')">
                            @csrf @method('DELETE')
                            <button data-action="delete" data-id="{{ $k->id }}" type="submit" class="text-red-500 hover:bg-red-100 p-1.5 rounded-lg">
